...
// TEST_CASE("перемещение") {
//     SUBCASE("Move constructor integrity") {

//         BitsetD S(1000, true);
//         S.set(500, false);
//         int32_t original_size = S.size();

//         BitsetD C(S); 
//         BitsetD R(std::move(S));

//         CHECK(0 == S.size()); // S должна стать пустой
//         CHECK(C == R);        // Данные должны полностью перейти в R
//         CHECK(R.size() == original_size);
//         CHECK(R.get(500) == false);
//     }
// }
// TEST_CASE("Move semantics and Performance") {
//     SUBCASE("Default constructor") {
//         BitsetD empty_bs;
//         CHECK(empty_bs.size() == 0);
//         BitsetD moved_empty(std::move(empty_bs));
//         CHECK(moved_empty.size() == 0);
//         CHECK(empty_bs.size() == 0);
//     }
// }
